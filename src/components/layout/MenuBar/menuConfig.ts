...
const staticSections: MenuSection[] = [
  {
    ...
    items: [
      {
        kind: 'item',
        label: 'Run Fixation Detection',
        onSelect: log('Analysis > Fixation'),
      },
      {
        kind: 'item',
        label: 'Run Saccade Detection',
        onSelect: log('Analysis > Saccade'),
      },
      { kind: 'item', label: 'Compute AOIs', onSelect: log('Analysis > AOIs') },
      {
        kind: 'item',
        label: 'Calibration Check',
        onSelect: log('Analysis > Calibration Check'),
      },
      { kind: 'separator' },
      {
        kind: 'item',
        label: 'Analysis Settings…',
        onSelect: log('Analysis > Settings'),
      },
    ],
  },
  {
    ...
    items: [
      { kind: 'item', label: 'Heatmap', onSelect: log('Visualize > Heatmap') },
      {
        kind: 'item',
        label: 'Scanpath',
        onSelect: log('Visualize > Scanpath'),
      },
      {
        kind: 'item',
        label: 'Gaze Plot',
        onSelect: log('Visualize > Gaze Plot'),
      },
      {
        kind: 'item',
        label: '3D Scene Overlay',
        onSelect: log('Visualize > 3D Scene Overlay'),
      },
      { kind: 'separator' },
      {
        kind: 'item',
        label: 'Visualization Settings…',
        onSelect: log('Visualize > Settings'),
      },
    ],
  },
  {
    ...
    items: [
      {
        kind: 'item',
        label: 'Documentation',
        onSelect: log('Help > Documentation'),
      },
      {
        kind: 'item',
        label: 'Keyboard Shortcuts',
        onSelect: log('Help > Shortcuts'),
      },
      {
        kind: 'item',
        label: 'Report Issue',
        onSelect: log('Help > Report Issue'),
      },
      { kind: 'separator' },
  ...
  const fileSection: MenuSection = {
    ...
    items: [
      ...
      },
      {
        kind: 'item',
        label: 'Exit',
        onSelect: () => window.api?.windowControls.close(),
      },
    ],
  };
